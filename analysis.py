...
def plot_derivative_thresholds_time(datasets):
    ...
    plt.errorbar(times, max_derivative2_voltages, yerr = errors, color = 'green', label="2nd derivative method")

def plot_steepest_threshold_time(datasets):
    x_intercepts = []
    errors = []
    times = range(0, len(datasets))
    for dataset in datasets:
        ...
        errors.append(error)
    plt.errorbar(times, x_intercepts, yerr = errors, color = 'red', label="steepest line method")
